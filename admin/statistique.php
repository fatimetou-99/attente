...
if (mysqli_num_rows($sql1)>0) {
  while ($row = mysqli_fetch_assoc($sql1)) {
    ...
    $insr = mysqli_query($con,"INSERT INTO statistique (servic, totale, jour, mois, annee,date) 
    VALUES ('Retrait',$tot1,'$jour','$mois','$annee','$dt1') ");
    
  }
}


  $sql2=mysqli_query($con,"SELECT COUNT(id_v) as total,date_v FROM versement GROUP BY date_v");
  ...
  if (mysqli_num_rows($sql2)>0) {
    while ($row = mysqli_fetch_assoc($sql2)) {
      ...
      $anneeV = dateToFrench($dt2,'Y');
        $insr = mysqli_query($con,"INSERT INTO statistique ( servic, totale, jour, mois,annee,date) 
        VALUES ('Versement',$tot2,'$jourV','$moisV','$anneeV','$dt2')");     
    }
  }
